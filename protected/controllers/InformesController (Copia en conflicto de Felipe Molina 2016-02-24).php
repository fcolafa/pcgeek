...
class InformesController extends Controller
{
	...
	{
		$detalle = DetalleInforme::model()->findAllBySql("SELECT `i`.`ID_INFORME_DET`, `i`.`ID_VISITA`, `v`.`TIPO_VISITA`, `v`.`NOMBRE_VISITA`, `i`.`ID_ESTADO` FROM `detalle_informe` `i` INNER JOIN `visitas` `v` ON `i`.`ID_VISITA`=`v`.`ID_VISITA` WHERE `i`.`ID_INFORME`=".$id);
