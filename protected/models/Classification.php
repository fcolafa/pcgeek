...
class Classification extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id_classification, classification_name', 'required'),
			array('id_classification', 'numerical', 'integerOnly'=>true),
			array('classification_name', 'length', 'max'=>45),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id_classification, classification_name', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'tickets' => array(self::HAS_MANY, 'Ticket', 'id_classification'),
		);
	}
