 ...
 * @property string $ticket_remedy_date
 *
...
class Ticket extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id_user, ticket_date, ticket_subject, ticket_description, ticket_date_incident, id_classification', 'required'),
			array('id_embarkation, id_user, id_headquarter, id_classification', 'numerical', 'integerOnly'=>true),
			array('ticket_subject, ticket_status', 'length', 'max'=>45),
			array('ticket_solution, other_classification, ticket_close_date, ticket_remedy_date', 'safe'),
			...
			array('id_ticket, id_embarkation, id_user, ticket_date, ticket_subject, ticket_description, ticket_status, ticket_date_incident, id_headquarter, ticket_solution, other_classification, id_classification, ticket_close_date, ticket_remedy_date', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'idUser' => array(self::BELONGS_TO, 'Usuarios', array('id_user'=>'ID_USUARIO')),
			'ticketFiles' => array(self::HAS_MANY, 'TicketFile', 'id_ticket'),
			'ticketMessages' => array(self::HAS_MANY, 'TicketMessage', 'id_ticket'),
		);
	}
	...
	public function search()
	{
		...
		$criteria=new CDbCriteria;
                $criteria->with=array('idUser.iDCLIENTE');
		...
		$criteria->compare('iDCLIENTE.NOMBRE_CLIENTE', $this->_client_name,true);
        ...
        public function searchClient()
	{
		...
		$criteria=new CDbCriteria;
                $criteria->with=array('idUser.iDCLIENTE');
                $criteria->together=true;
                $criteria->condition = 't.id_user='.Yii::app()->user->getUser_Id();
		$criteria->compare('id_ticket',$this->id_ticket);
		//$criteria->compare('id_embarkation',$this->id_embarkation);
		$criteria->compare('id_user',$this->id_user);
		$criteria->compare('ticket_date',$this->ticket_date,true);
